...
export class StringView {
  static formatLoggerSintaxe(text: string, args: unknown[], kwargs: Partial<Record<string, unknown>>): string {
    if (!args && !kwargs) {
      return text;
    }
      ...
      if (current === '%') {
        ...
        if (key === 's') {
          ...
          if (currentArgValue === undefined) {
            throw new Error("List out the range.");
          }

          currentArgIndex += 1
          if (typeof currentArgValue === 'string') {
            finalText += currentArgValue
          } else {
            finalText += inspect(currentArgValue)
          }
